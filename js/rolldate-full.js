 ...
 * Copyright 2025
 *  
 * modified by: xianbochen@gmailcom
 * 日期滚动选择控件。同时支持公农历四柱日期切换选择。
 */
(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() :
        typeof define === 'function' && define.amd ? define(factory) :
            (global = global || self, global.RolldateFull = factory());
}(this, function () {
    'use strict';
    ...
    RolldateFull.prototype = {
        constructor: RolldateFull,
        ...
        initValue: function initValue() {
            ...
            _this.date = date;
            _this.lunar = lunar;
            if( config.dateType==3 ){
                var bazi = lunar.getEightChar();
                var bz = [bazi.getYearGan(), bazi.getYearZhi(), bazi.getMonthGan(), bazi.getMonthZhi(), bazi.getDayGan(), bazi.getDayZhi(), bazi.getTimeGan(), bazi.getTimeZhi()];
                this.config.bazi = bz;//8个字的数组
                this.config.zhaowanzhishi = config.zhaowanzhishi;//是否早晚子时
            }
        },
        ...
        baseData: function baseData() {
            return {
                ...
                opts: { //插件默认配置
                    ...
                    minStep: 1,
                    trigger: 'tap',
                    lang: { cancel: '取消', confirm: '确认', year: '年', month: '月', day: '日', hour: '时', min: '分', sec: '秒' }
                }
            };
        },
        ...
        createUI: function createUI() {
            var _this = this,
                ...
                ul = '',
                date = _this.date || new Date(),
                itemClass = '',
                lang = config.lang;
            var lunar = Lunar.fromDate(date);
            var lunarYear = LunarYear.fromYear(date.getFullYear());

            if( !document.getElementById("rolldate-content") ){
                var $html = `
                            <div class="rolldate-mask"></div>
                            <div class="rolldate-panel fadeIn">
                                <header style="height:60px">
                                    <span class="rolldate-btn rolldate-cancel">` + lang.cancel + `</span>
                                        <span class="rolldate-button-date-group" style="text-align:center">
                                            <span class="rolldate-button-date rolldate-button-date-gongli rolldate-button-date-active">公历</span>
                                            <span class="rolldate-button-date rolldate-button-date-nongli">农历</span>
                                            <span class="rolldate-button-date rolldate-button-date-shizhu">四柱</span>
                                        </span>                                        
                                    <span class="rolldate-btn rolldate-confirm">` + lang.confirm + `</span>
                                </header>
                                <section id="rolldate-content" class="rolldate-content">
                                    <div class="rolldate-dim mask-top"></div>
                                    <div class="rolldate-dim mask-bottom"></div>
                                    <div id="rolldate-wrapper" class="rolldate-wrapper">
                                    </div>
                                </section>
                            </div>`,
                ...
                $(".shizhudate-gan-picker").remove();
                $(".shizhudate-zhi-picker").remove();
                var ganList = ["甲","乙","丙","丁","戊","己","庚","辛","壬","癸"];
                var ganHtml = "";
                for (var gi = 0; gi < ganList.length; gi++) {
                    ganHtml += "<div class='shizhudate-picker-cell " + wuxingStyle(tianganWuxing(ganList[gi])) + "'>" + ganList[gi] + "</div>";
                }
                ...
                var zhiHtml = "";
                for (var zi = 0; zi < zhiList.length; zi++) {
                    zhiHtml += "<div class='shizhudate-picker-cell " + wuxingStyle(dizhiWuxing(zhiList[zi])) + "'>" + zhiList[zi] + "</div>";
                }
                ...
                $(".rolldate-button-date").on("click", function () {
                    if ($(this).attr("class").indexOf("gongli") != -1) {//切换为公历
                        _this._activeDateType(1);
                        _this.show();
                    } else if ($(this).attr("class").indexOf("nongli") != -1) {//切换为农历
                        _this._activeDateType(2);
                        _this.show();
                    } else if ($(this).attr("class").indexOf("shizhu") != -1) { //切换为四柱
                        _this._activeDateType(3);
                        _this.show();
                    }
                });
            }
            
            if (this.config.dateType!=3 ){
                $(".mask-top").show();
                $(".mask-bottom").show();
                $("#rolldate-wrapper").removeClass();
                $("#rolldate-wrapper").addClass("rolldate-wrapper");
                this._createDateElements(FormatArr, domId, date, lunar, lunarYear, config, lang);
                this._initScrollEvents(FormatArr, domId, date, config, lang);
            }else{
                $(".mask-top").hide();
                $(".mask-bottom").hide();
                $("#rolldate-wrapper").removeClass();
                $("#rolldate-wrapper").addClass("shizhudate-content");
                this._createShizhuElements();
            }
            _this._activeDateType(this.config.dateType);

        },
        _activeDateType: function(dateType){
            this.config.dateType = dateType;
            if (dateType==1) {//切换为公历
                $(".rolldate-button-date").removeClass("rolldate-button-date-active");
        ...
        _createShizhuElements: function(){
            var shizhuHtml = `
            ...
            $("#rolldate-wrapper").html(shizhuHtml);

            if( !this.config.bazi || this.config.bazi.length!=8 ){
                var bazi = Lunar.fromDate(new Date()).getEightChar();
                var bz = [bazi.getYearGan(), bazi.getYearZhi(), bazi.getMonthGan(), bazi.getMonthZhi(), bazi.getDayGan(), bazi.getDayZhi(), bazi.getTimeGan(), bazi.getTimeZhi()];
                this.config.bazi = bz;//8个字的数组
            }
        ...
        _createDateElements: function(FormatArr, domId, date, lunar, lunarYear, config, lang) {
            var _this = this;
            var ul = '';
            ...
            for (var i = 0; i < len; i++) {
                ...
                if (f == 'YYYY') {
                    for (var j = config.beginYear; j <= config.endYear; j++) {
                        ...
                        var dv = j;
                        ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '" dv="'+dv+'">' + dv + lang.year + '</li>';
                        domMndex++;
                    }
                } else if (f == 'MM') {
                    var lunarMonths = null;
                    if( config.dateType==2 ){
                        lunarMonths = lunarYear.getMonths();
                        for (var k = 0; k < lunarMonths.length; k++) {
                            var l = Lunar.fromYmd(lunarYear.getYear(), lunarMonths[k].getMonth(), 1);
                            var mVal = l.getMonthInChinese() + lang.month;
                            itemClass = lunar.getMonth() == l.getMonth() ? 'active' : '';
                            ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '" dv="'+l.getMonth()+'">' + mVal + '</li>';
                            domMndex++;
                        }
                    }else if( config.dateType==1 ){
                        for (var k = 1; k <= 12; k++) {
                            itemClass = k == date.getMonth() + 1 ? 'active' : '';
                            var dv = (k < 10 ? '0' + k : k) ;
                            ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '" dv="'+dv+'">' + dv + lang.month + '</li>';
                            domMndex++;
                        }
                    }
                } else if (f == 'DD') {
                    if( config.dateType==2 ){
                        var dCount = lunarYear.getMonth(lunar.getMonth()).getDayCount();
                        for( var d = 1; d<=dCount; d++ ){
                            var l = Lunar.fromYmd(lunarYear.getYear(), lunar.getMonth(), d);
                            var sl = l.getSolar();
                            var newDate = new Date(sl.getYear(), sl.getMonth()-1, sl.getDay(), sl.getHour(), sl.getMinute(), 0);
                            itemClass = newDate.getDate() == date.getDate() ? 'active' : '';
                            ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '" dv="'+l.getDay()+'">' + l.getDayInChinese() + '</li>';
                            domMndex++;
                        }
                    }else if( config.dateType==1 ){
                        ...
                        for (var l = 1; l <= day; l++) {
                            ...
                            var dv = (l < 10 ? '0' + l : l);
                            ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '" dv="'+dv+'">' + dv + lang.day + '</li>';
                            domMndex++;
                        }
                    }
                } else if (f == 'hh') {
                    for (var m = 0; m <= 23; m++) {
                        ...
                        ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '" dv="'+dv+'">' + dv + dn + lang.hour + '</li>';
                        domMndex++;
                    }
                } else if (f == 'mm') {
                    for (var n = 0; n <= 59; n += config.minStep) {
                        ...
                        var dv = (n < 10 ? '0' + n : n);
                        ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '" dv="'+dv+'">' + dv + lang.min + '</li>';
                        domMndex++;
                    }
                } else if (f == 'ss') {
                    for (var o = 0; o <= 59; o++) {
                        ...
                        var dv = (o < 10 ? '0' + o : o);
                        ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '" dv="'+dv+'">' + dv + lang.sec + '</li>';
                        domMndex++;
                    }
                }
                ul += '</ul></div>';
            }
            document.getElementById("rolldate-wrapper").innerHTML = ul;
        },
        
        // 新增的函数：初始化滚动事件
        _initScrollEvents: function(FormatArr, domId, date, config, lang) {
            var _this = this;
            var len = FormatArr.length;
            ...
            var _loop = function _loop(_i) {
                ...
                that.on('scrollEnd', function () {
                    ...
                    if( config.dateType==2 ){
                        if ( [domId['YYYY']].indexOf(that.wrapper.id) != -1 ) {
                            var y = parseInt(_this.getSelected(_this.scroll['YYYY']));
                            var d = 1;
                            var lunarYear = LunarYear.fromYear(y);
                            var lunarMonths = lunarYear.getMonths();
                            var li = '';
                            for( var k=0;k<lunarMonths.length;k++ ){
                                var l = Lunar.fromYmd(y, lunarMonths[k].getMonth(), 1);
                                var mVal = l.getMonthInChinese() + lang.month;
                                li += '<li class="wheel-item" dv="'+lunarMonths[k].getMonth()+'">' + mVal + '</li>';
                            }
                            _this.$('#' + domId['MM'] + ' ul').innerHTML = li;
                            _this.scroll['MM'].refresh();
                        }else if ( [domId['YYYY'], domId['MM']].indexOf(that.wrapper.id) != -1 ) {
                            var y = parseInt(_this.getSelected(_this.scroll['YYYY']));
                            var m = parseInt(_this.getSelected(_this.scroll['MM']));
                            var lr = Lunar.fromYmd(y, m, 1);
                            var lunarYear = LunarYear.fromYear(y);
                            var dCount = lunarYear.getMonth(lr.getMonth()).getDayCount();
                            var li = "";
                            for( var d = 1; d<=dCount; d++ ){
                                var l = Lunar.fromYmd(y, m, d);
                                li += '<li class="wheel-item" dv="'+l.getDay()+'">' + l.getDayInChinese() + '</li>';
                            }
                        ...
                        if ([domId['YYYY'], domId['MM']].indexOf(that.wrapper.id) != -1 && _this.scroll['DD']) {
                            ...
                            var _days = _this.bissextile(_this.getSelected(_this.scroll['YYYY']), _this.getSelected(_this.scroll['MM']));
                            var li = '';
    
                            if (_days != _this.$('#' + domId['DD'] + ' li', 1).length) {
    
                                for (var _l = 1; _l <= _days; _l++) {
                                    var dv = (_l < 10 ? '0' + _l : _l);
                                    li += '<li class="wheel-item" dv="'+dv+'">' + dv + lang.day + '</li>';
                                }
                                _this.$('#' + domId['DD'] + ' ul').innerHTML = li;
                                _this.scroll['DD'].refresh();
                            }
        ...
        tap: function tap(el, fn) {
            ...
            if (_this.config.dateType!=3 && hasTouch && _this.config.trigger == 'tap') {
                var o = {};
        ...
        show: function show(type) {
            ...
            if( arguments && arguments.length == 1 ) {
                config.dateType = type||1;
            }

            if( config.showValue ){
                var showValue = config.showValue();
                config.value = showValue.dateValue||config.value;
                config.zhaowanzhishi = showValue.wanzhishi;
                _this.initValue();
            }
        ...
        event: function event() {
            ...
            if( confirm ){
                _this.tap(confirm, function () {
                    ...
                    if (config.dateType==1){
                    ...
                    }else if( config.dateType==2 ){
                        var yy,mm,dd,hh,mi,ss;//for lunar use.
                        for (var f in _this.scroll) {
                            var x = _this.getSelected(_this.scroll[f]);
                            if (f == 'YYYY') {
                                yy = parseInt(x);
                            } else if (f == 'MM') {
                                mm = parseInt(x);
                        ...
                        var newLunar = Lunar.fromYmdHms(yy,mm,dd,hh,mi,0);
                        var sl = newLunar.getSolar();
                        newDate = new Date(sl.getYear(), sl.getMonth()-1, sl.getDay(), sl.getHour(), sl.getMinute(), 0);
                        if (config.confirm) {
                            var flag = config.confirm.call(_this, newDate);
                            if (flag === false) {
                                return false;
                        ...
                        var dateStr = layui.util.toDateString(newDate, "yyyy-MM-dd HH:mm");
                        if (el.nodeName.toLowerCase() == 'input') {
                            el.value = dateStr;
                        } else {
                        ...
                        var days = Solar.fromBaZi(
                            ...
                            $("#bz5").text()+$("#bz6").text(), 
                            $("#bz7").text()+$("#bz8").text(),
                            _this.config.zhaowanzhishi?2:1, 1800);
                        ...
                        days.reverse().forEach(function(d){
                            itemHtml += "<div class='shizhudate-result-item'>"+d.toYmdHms().slice(0, -3)+"</div>";
                        });
                        $(".shizhudate-result-item").off("click", confirmShizhuResult.bind(_this));
            ...
            if( _this.config.dateType==3 && ganzhi ){
                _this.tap(ganzhi, function(){
                    showGanzhiPicker(_this, this);
                });
            }
            if( _this.config.dateType==3 && gzpick){
            ...
            var confirmShizhuResult = function(e){
                ...
                var el = this.$(this.config.el);
                if (el.nodeName.toLowerCase() == 'input') {
                    el.value = selectDate;
                } else {
                    el.innerText = selectDate;
                }
                this.hide();
            }
        ...
        getSelected: function getSelected(scroll) {
            return this.$('#' + scroll.wrapper.id + ' li', 1)[scroll.getSelectedIndex()].getAttribute("dv");
        }
    };
    RolldateFull.version = version;
