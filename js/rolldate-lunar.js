        ...
        if (config.value) {
            if( config.isLunar ){
                var lunar = Lunar.fromYmdHms(
                    ...
                    config.value.mi,
                    config.value.ss);
                var lrStr = newLunar.toString() + " " + (hh<10?"0"+hh:hh) + ":" + (mi<10?"0"+mi:mi);
                if (el.nodeName.toLowerCase() == 'input') {
                    el.value = lrStr;
                } else {
                    el.innerText = lrStr;
                }
                el.bindDate = lunar.getSolar();
                el.bindLunar = lunar;
    ...
    Rolldate.prototype = {
                        ...
                        <div class="rolldate-panel fadeIn">
                            <header>
                                <span class="rolldate-btn rolldate-cancel">` + lang.cancel + `</span>
                                    <span id="rolldate-title">` + lang.title + `</span>
        ...
        show: function show() {
            ...
            if( config.isLunar ){
                ...
                var dValue = $("#mydate").val(); //FIXME: 想办法直接取当前el的值
                var str = dValue.replace(/-/g, '/').replace(/[^\d/:\s]/g, '');
                var aDate = new Date(str);
                var newLunar = Lunar.fromDate(aDate);
                var sl = newLunar.getSolar();
                var newDate = new Date(sl.getYear(), sl.getMonth()-1, sl.getDay(), sl.getHour(), sl.getMinute(), 0);
                el.bindDate = newDate;
                el.bindLunar = newLunar;
        ...
        event: function event() {
            ...
            _this.tap(confirm, function () {
                ...
                if( config.isLunar ){
                    ...
                    if (config.confirm) {
                        var flag = config.confirm.call(_this, newDate, newLunar);
                        if (flag === false) {
                            return false;
                        } else if (flag) {
                            date = flag;
